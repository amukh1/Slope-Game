var clicky_obj = clicky_obj || function () {
  ...
  function _ins() {
    ...
    this.custom_data = function () {
      var data = {}, keys = ["username", "name", "email"], l = keys.length;
      for (var i = 0; i < l; i++) {
        var key = keys[i], temp = "";
      ...
      if (clicky_custom.visitor) {
        for (var i in clicky_custom.visitor) {
          if (clicky_custom.visitor.hasOwnProperty && clicky_custom.visitor.hasOwnProperty(i)) if (!data[i]) data[i] = clicky_custom.visitor[i];
        }
      }
      if (data) {
        for (var i in data) {
    ...
    this.doc_wh = function () {
      ...
      return {w: window.innerWidth || de.clientWidth || 1024, h: Math.max(db.scrollHeight, db.offsetHeight, de.clientHeight, de.scrollHeight, de.offsetHeight)};
    };
    this.heatmap = function (date, sub, subitem) {
      if (window._heatmap_destroy) _heatmap_destroy();
      if (window.heatmapFactory) _self.heatmap_data(date, sub, subitem); else {
        _self.inject("//static.getclicky.com/inc/javascript/heatmap.js");
        setTimeout('_cgen.heatmap("' + (date || "") + '","' + (sub || "") + '","' + (subitem || "") + '")', 1e3);
      }
    };
    ...
    this.onsitestats = function (refresh, reset) {
      if (ossassets) {
        ...
        setTimeout(_self.onsitestats, 1e3);
      }
    };
    ...
    this.hm_monitor = function () {
      ...
      } else setTimeout(_self.hm_monitor, 1e3);
    };
    ...
    this.goal = function (id, revenue) {
      ...
      _self.beacon({type: "goal", q: "&goal" + goal + (revenue ? "&goal[revenue]=" + revenue : "")});
    };
    this.beacon = function (type, q, called_by_pageview) {
      if (typeof type == "object") {
        var o = type;
        if (o.type) type = o.type; else return false;
        if (o.q) {
          q = o.q;
        } else {
          var temp = "";
      ...
      var custom = "", goal = "", split = "", jsuid = _self.get_cookie("_jsuid");
      ...
      for (var site_id_index = 0; site_id_index < site_ids.length; site_id_index++) {
        var site_id = site_ids[site_id_index], hm = _self.get_cookie("_heatmaps_g2g_" + site_id) || window["_heatmaps_g2g_" + site_id] || "";
    ...
    this.is_pageview_fired = function (site_id) {
      for (var p = 0; p < pageviews_fired.length; p++) if (pageviews_fired[p] == site_id) return true;
      return false;
    };
    this.ping_start = function () {
      ...
      setTimeout(_self.ping, 3e4);
      setTimeout(_self.ping, 6e4);
      setTimeout(_self.ping_set, 12e4);
    };
    this.ping_set = function () {
      var pingy = setInterval(_self.ping, 12e4);
      setTimeout(function () {
        clearInterval(pingy);
      }, _self.ps_stop * 1e3);
      _self.ping();
    ...
    this.set_cookie = function (name, value, expires, force) {
      ...
      var maxage = expires == "session" ? "" : ";max-age=" + (expires || 31536e3);
...
var _cgen = clicky, _cgen_custom = clicky_custom;
_cgen.init(101357250);
